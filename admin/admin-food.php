        ...
        <div class="row">
            <div class="col-md-12">
                <table class="content-table">
                  ...
                  <?php
                      if($state_id == 0){
                        $sql = "SELECT food_id, name, donor_name, email, cooking_timing, phone, state_name, full_address, state_name, state_id FROM food INNER JOIN state ON
                        state.state_id = food.state WHERE food.collect = ?
                        ORDER BY food_id DESC LIMIT ?, ?";
                        $values = array(0 ,$offset, $limit);
                      } else {
                        $sql = "SELECT food_id, name, donor_name, email, cooking_timing, phone, state_name, collect, full_address, state_name, state_id FROM food INNER JOIN state ON
...
<div class="w3-row pagination">
  ...
  if($state_id == 0){
    ...
    INNER JOIN state ON food.state = state.state_id WHERE food.collect = ?";
    ...
    if($result2->rowCount()){
      ...
      echo '<ul>';
      if($page > 1){
        echo '<li class="w3-button w3-green"><a href="admin-food.php?state='.$state_id.'&page='.($page-1).'" class="w3-blue p-3">Previous</a></li>';
      }
      ...
      for($i = 1; $i <= $total_pages; $i++){
        ...
        echo '<li class="w3-button w3-green '.$active.'"><a href="admin-food.php?state='.$state_id.'&page='.$i.'" class="w3-blue p-3" style="padding:100%;">'.$i.'</a></li>';
      }
      if($total_pages > $page){
        echo '<li class="w3-button w3-green"><a href="admin-food.php?state='.$state_id.'&page='.($page + 1).'" class="w3-blue p-3">Next</a></li>';
      }
      echo '</ul>';
    }
  ?>
